...
def post_AMS_alarm_message(mode):

    
    parseCfg = ParseSetting()
    AMSAlarm = parseCfg.read('AMSAlarm')
    AMSSwitch = AMSAlarm['switch']
    alarmSubject = AMSAlarm['alarmSubject']
    alarmContent = AMSAlarm['handsContent'] if mode == 'hands' else AMSAlarm['motionContent']
    ...
    f"        <AlarmSubject>{alarmSubject}</AlarmSubject>\r\n"\
    f"        <AlarmContent>{alarmContent}</AlarmContent>\r\n"\
...
class Hands:
    ...
    def __init__(self, model, recordPath, pixelThres, degree):
        ...
        self.skipFrameFlag = False
        self.lastDateText = 'init'
        self.preMaskBoxes = []
        self.recordPath = recordPath
        self.overlapCounts = 0
        self.secAlarmLock = False
    ...
    def run(self, image):
        ...
        if self.NGFlag and centerFlag:
            ...
            if dateText == self.lastDateText and  AlarmInterval >= 3:   
                    ...
                    for box1 in maskBoxes:
                        for box2 in self.preMaskBoxes:
                            ...
                            if boxIOU >= IOUThresh:    # continuous touching
                                ...
                                if self.overlapCounts >= overlapThresh:
                                    judgeFlag = True
                                    self.overlapCounts = 0
